...
compiler_flags = """ --target=trn2 --lnc=2 --model-type=transformer --auto-cast=none --enable-fast-loading-neuron-binaries --internal-hlo2tensorizer-options='--remat-by-default' """
...
def _pad_to_multiple_neginf(x, dim, multiple):
    """Pad tensor along dim to the nearest multiple with -inf. Returns (padded, original_len)."""
    orig_len = x.shape[dim]
    remainder = orig_len % multiple
    if remainder == 0:
        return x, orig_len
    ...
    pad_shape = list(x.shape)
    pad_shape[dim] = pad_len
    padding = torch.full(pad_shape, float("-inf"), dtype=x.dtype, device=x.device)
    return torch.cat([x, padding], dim=dim), orig_len


def nki_flash_attention(query, key, value):
    """
    NKI Flash Attention wrapper with zero pre-padding for non-aligned sequences.

    When sequence lengths are not multiples of 128 (NKI tile size), we pre-pad
    Q, K, V with zeros to the next multiple and trim the output.

    Zero-padding K means padded positions get exp(0)=1 in softmax, causing a small
    dilution of attention weights. At 720P this is 80/18560 = 0.43% dilution per
    head per layer — empirically negligible vs the 2.6x speedup from using the
    NKI kernel instead of SDPA fallback.
    ...
    q_needs_pad = (q_len % NKI_SEQ_TILE) != 0
    k_needs_pad = (k_len % NKI_SEQ_TILE) != 0

    # Pre-pad to tile-aligned lengths if needed (zero padding)
    if q_needs_pad:
        query, _ = _pad_to_multiple(query, dim=2, multiple=NKI_SEQ_TILE)
    if k_needs_pad:
    ...
    v = value.clone().reshape((bs * n_head, padded_k_len, d_head))
    ...
    # Trim padding from output if Q was padded
    if q_needs_pad:
    ...
    def forward(
        ...
        if self.context_parallel_enabled:
            ...
            if self.cfg_parallel:
                ...
                # Split per-position timestep_mask along seq dim for CP
                if timestep_mask is not None:
                    timestep_mask = split_along_dim(
                        timestep_mask, dim=1, rank=dp_rank, data_parallel_group=dp_group
                    )

        # Dual-timestep approach for I2V: compute embeddings for t=0 and t=current separately,
        # then blend per-position using timestep_mask.
        # This avoids [batch, seq_len, dim] intermediates that cause OOM at 720P.
        #
        # timestep_mask: [batch, seq_len] where 0=clean (frame 0), 1=noisy (other frames)
        # For T2V: all ones (or None). For I2V: frame 0 positions are 0.

        # Check if we need per-position conditioning (I2V)
        use_perpos = timestep_mask is not None

        if use_perpos:
            # Must compute t=0 embedding FIRST, before encoder_hidden_states is transformed.
            # condition_embedder applies text_embedder to encoder_hidden_states (4096 -> 3072),
            # so we can only call it once with the raw encoder_hidden_states.
            # Strategy: compute t=0 first (transforms enc_hs), then compute t=t using
            # the already-transformed enc_hs by only running timestep path.
            timestep_zero = torch.zeros_like(timestep)
            temb_0, proj_0, encoder_hidden_states, _ = self.condition_embedder(
                timestep_zero, encoder_hidden_states, None
            )
            proj_0 = proj_0.unflatten(1, (6, -1))  # [batch, 6, dim]

            # Now encoder_hidden_states is already transformed (3072 dim).
            # For timestep t, we only need the timestep embedding path,
            # NOT the text_embedder path again.
            # Extract timestep embedding manually:
            t_proj = self.condition_embedder.timesteps_proj(timestep)
            t_emb_dtype = next(
                iter(self.condition_embedder.time_embedder.parameters())
            ).dtype
            if t_proj.dtype != t_emb_dtype and t_emb_dtype != torch.int8:
                t_proj = t_proj.to(t_emb_dtype)
            temb_t = self.condition_embedder.time_embedder(t_proj).type_as(
                encoder_hidden_states
            )
            proj_t = self.condition_embedder.time_proj(
                self.condition_embedder.act_fn(temb_t)
            )
            proj_t = proj_t.unflatten(1, (6, -1))  # [batch, 6, dim]

            # Reshape mask for broadcasting: [batch, seq_len] -> [batch, seq_len, 1]
            mask_3d = timestep_mask.unsqueeze(-1)  # [batch, seq_len, 1]
        else:
            # T2V: single call suffices
            temb_t, proj_t, encoder_hidden_states, _ = self.condition_embedder(
        ...
        for block in self.blocks:
            if use_perpos:
                # Compute scale/shift for t and for 0
                params_t = (block.scale_shift_table + proj_t.float()).chunk(6, dim=1)
                params_0 = (block.scale_shift_table + proj_0.float()).chunk(6, dim=1)
                # Blend per-position: mask=1 -> use params_t, mask=0 -> use params_0
                # params are [batch, 1, dim], mask_3d is [batch, seq_len, 1]
                shift_msa = params_0[0] + mask_3d * (params_t[0] - params_0[0])
                scale_msa = params_0[1] + mask_3d * (params_t[1] - params_0[1])
                gate_msa = params_0[2] + mask_3d * (params_t[2] - params_0[2])
                c_shift_msa = params_0[3] + mask_3d * (params_t[3] - params_0[3])
                c_scale_msa = params_0[4] + mask_3d * (params_t[4] - params_0[4])
                c_gate_msa = params_0[5] + mask_3d * (params_t[5] - params_0[5])
            else:
                # Scalar T2V: [batch, 6, dim] -> 6 chunks of [batch, 1, dim]
                shift_msa, scale_msa, gate_msa, c_shift_msa, c_scale_msa, c_gate_msa = (
                    block.scale_shift_table + proj_t.float()
                ).chunk(6, dim=1)
        ...
        if use_perpos:
            # Blend temb for output norm
            temb_blend = temb_0.unsqueeze(1) + mask_3d * (
                temb_t.unsqueeze(1) - temb_0.unsqueeze(1)
            )
            shift, scale = (
                self.scale_shift_table.unsqueeze(0).to(temb_blend.device)
                + temb_blend.unsqueeze(2)
            ).chunk(2, dim=2)
            shift = shift.squeeze(2)
            scale = scale.squeeze(2)
        else:
            shift, scale = (
                self.scale_shift_table.to(temb_t.device) + temb_t.unsqueeze(1)
            ).chunk(2, dim=1)
        hidden_states = (
...
def fix_norm_weights_per_rank(weights_path, unsharded_norm_weights, tp_degree):
    ...
    for rank in range(tp_degree):
        ...
        ckpt = {k: v.clone() for k, v in load_file(ckpt_path).items()}
...
def compile_transformer_v3_cp(args):
    ...
    # Mask for I2V: [batch, seq_len] where 0=clean frame 0, 1=noisy other frames
    # For T2V, pass all-ones; for I2V, frame 0 positions are 0
    sample_timestep_mask = torch.ones(batch_size, seq_len, dtype=torch.float32)
