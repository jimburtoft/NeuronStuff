...
if __name__ == '__main__':
    ...
    if os.path.isfile(model_file):
        ...
        def get_model():
            ...
            torch.save({"model": model.state_dict()}, "MBERT.pt")
            model.eval()
            neuronx_distributed.parallel_layers.load("MBERT.pt", model, sharded=False)
            io_aliases = {}
        ...
        try:
            traced_model = neuronx_distributed.trace.parallel_model_trace(
                get_model,
                paraphrase,
                tp_degree=1,
            ...
            raise
        neuronx_distributed.trace.parallel_model_save(traced_model, model_file)
    ...
    example = paraphrase
    # Load models
    models = [neuronx_distributed.trace.parallel_model_load(model_file) for _ in range(n_models)]
