"""
Wan 2.2 T2V-A14B Preloadable Subprocess Worker
================================================
Two-phase worker:
  Phase 1 (preload): Load model + weights -> write "ready" signal
  Phase 2 (denoise): Wait for input data -> denoise -> save output

This allows overlapping model load of Expert 2 with Expert 1's denoising.
Neuron RT environment variables are passed via neuron_env dict.

Usage:
    python worker_denoise_preload.py <workdir>

Workdir contains:
    - preload.pt:  {compiled_path, pipe_transformer_state, neuron_env}
    - ready:       Created by worker when model is loaded
    - input.pt:    Written by orchestrator when ready for denoise
    - output.pt:   Written by worker when denoise is done
"""
...
POLL_INTERVAL = 0.5  # seconds between checking for input file
...
def main():
    ...
    # ========== PHASE 1: PRELOAD ==========
    print(f"[PreloadWorker] Loading preload data from {preload_path}")
    preload = torch.load(preload_path, weights_only=False)

    # Setup Neuron env BEFORE importing torch_neuronx
    setup_neuron_env(preload["neuron_env"])
    ...
    compiled_path = preload["compiled_path"]
    ...
    # Load weights
    print(f"[PreloadWorker] Loading weights (TP={tp_degree}, CP={cp_degree})...")
    t_load_start = time.time()
    tp_ckpts = load_sharded_weights(compiled_path, tp_degree)
    if preload.get("pipe_transformer_state") is not None:
        fix_norm_weights(tp_ckpts, preload["pipe_transformer_state"])
    ...
    print(f"[PreloadWorker] Loading NxDModel...")
    ...
    print(f"[PreloadWorker] Model loaded in {load_time:.1f}s")
    ...
    with open(ready_path, "w") as f:
        f.write(f"{load_time:.1f}")
    print(f"[PreloadWorker] READY signal written. Waiting for input...")

    # Free preload data
    del preload
    gc.collect()

    # ========== PHASE 2: WAIT FOR INPUT + DENOISE ==========
    ...
    print(
        f"[PreloadWorker] Running {num_steps} denoising steps "
    ...
    print(
        f"[PreloadWorker] Done: {num_steps} steps in {denoise_time:.1f}s "
    ...
    print(f"[PreloadWorker] Saved output to {output_path}")
